...
double suma_szeregu(double x, double dokladnosc, int M, int *warunek_stopu, int *liczba_wyrazow) {
    ...
    do {
        ...
        y++;
    } while (fabs(wartosc) >= dokladnosc && y <= M);
    ...
    *warunek_stopu = 0;
    ...
    if (y > M && fabs(wartosc) <= dokladnosc) {
        *warunek_stopu = 2;
    }
    return suma;
}
...
int main() {
    ...
    for (int i = 0; i <= n_czesci; i++) {
        ...
        else if (warunek_stopu == 1 ) {
            fprintf(plik, " %2.5lf \t%2.5lf \t%2.5lf \t\t%d \t\t%s\n", x, wartosc_szeregu, wartosc_dokladna, liczba_wyrazow, "maksymalne_M");
        }
        else {
            fprintf(plik, " %2.5lf \t%2.5lf \t%2.5lf \t\t%d \t\t%s\n", x, wartosc_szeregu, wartosc_dokladna, liczba_wyrazow, "osiagnieto_dokladnosc");
        }
    }
